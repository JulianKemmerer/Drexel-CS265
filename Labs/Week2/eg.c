...
find ~/mail -exec grep Heidi {} \; -print

	# find and delete all backups left by vim (*~) in all subdirectories:
find . -name "*~" -exec \rm {} \;

	# find files out in /tmp that should be cleaned up.
find /tmp -user $USER -mtime +1 -exec \rm -i {} \;
...
if [[ -f $listFile ]] ; then
	...
	# for user in `cat $listFile` ; do
		echo "Mailing $user..."
		#mail -s "Close class directories!" < /dev/null
	done
fi
